    ...
    if user_id == ADMIN_ID:
        ...
        if text == "🎬 Kino qo‘shish":
            ...
            msg = (f"📊 **Bot Statistikasi:**\n\n"
                   ...
                   f"📢 Kanallar: {c_count} ta\n"
                   f"👁 Jami ko'rishlar: {v_total} marta")
        ...
        if step == "rek_soni":
            if text.isdigit():
                ...
                if limit > total:
                    await update.message.reply_text(f"❌ Botda buncha user yo'q! Maksimal: {total}")
                else:
                    context.user_data["rek_limit"] = limit
                    context.user_data["step"] = "rek_xabar"
                    await update.message.reply_text(f"Yaxshi, {limit} kishiga yuboriladigan xabarni (rasm, video yoki matn) yuboring:")
            else:
            ...
            sent = 0
            msg = await update.message.reply_text("🚀 Reklama yuborilmoqda...")
            for u in users_list:
                try:
                    await update.message.copy_to(chat_id=u[0])
                    sent += 1
                    if sent % 10 == 0: await msg.edit_text(f"🚀 Jarayon: {sent}/{limit}")
                    await asyncio.sleep(0.05)
                except: pass
            
            await update.message.reply_text(f"✅ Reklama {sent} kishiga muvaffaqiyatli yuborildi!", reply_markup=admin_keyboard())
            ...
            conn.commit()
            await update.message.reply_text(f"✅ {text} kanal qo'shildi!", reply_markup=admin_keyboard())
            context.user_data.clear()
            return
            ...
            conn.commit()
            await update.message.reply_text(f"❌ {text} kanal o'chirildi!", reply_markup=admin_keyboard())
            context.user_data.clear()
            return
