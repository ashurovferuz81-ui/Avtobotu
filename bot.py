...
# ================= START (TUGMA TUZATILDI) =================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    user_id = str(user.id)
    username = user.username or "NoName"

    # Callback bo‘lsa doim answer qilamiz
    ...
    if not_joined:
        buttons = [
            [InlineKeyboardButton("📢 A'zo bo'lish", url=f"https://t.me/{c[1:]}" if c.startswith("@") else c)]
            for c in not_joined
        ]
        ...
        if update.callback_query:
            await update.callback_query.message.edit_text(
                "❌ Hali hamma kanallarga a'zo emassiz!",
                reply_markup=markup
            )
        else:
