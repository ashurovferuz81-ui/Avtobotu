...
async def messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        ...
        if step == "del_movie" and text:
            cur.execute("SELECT * FROM movies WHERE code=?", (text,))
            movie = cur.fetchone()
            if movie:
                cur.execute("DELETE FROM movies WHERE code=?", (text,))
                conn.commit()
                await update.message.reply_text(f"✅ Kod: {text} bo'lgan kino o'chirildi!")
            else:
