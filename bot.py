...
# ================= START (TUGMA MUAMMOSI TUZATILDI) =================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # CallbackQuery yoki oddiy Message ekanini aniqlash
    is_callback = update.callback_query is not None
    query = update.callback_query

    # Admin bo'lsa darhol panelni chiqarish
    if int(user_id) == ADMIN_ID:
        if is_callback: await query.answer()
        msg = query.message if is_callback else update.message
    ...
    for ch in all_channels:
        ...
        if ch.startswith("@"):
            try:
                ...
                not_joined.append(ch)
        # Linklar (https://) bu yerda not_joined ga qo'shilmaydi (ya'ni bot ularni tekshirmaydi)

    # Agar hali a'zo bo'lmagan @ kanallari bo'lsa
    if not_joined:
        buttons = []
        ...
        if is_callback:
            # Tugma bosilganda hali a'zo bo'lmasa
            await query.answer("❌ Siz hali hamma kanallarga a'zo bo'lmadingiz!", show_alert=True)
        else:
            # Birinchi marta /start bosganda
    ...
    # OBUNA TO'LIQ BO'LSA
    if is_callback:
        await query.answer("✅ Rahmat! Kirishga ruxsat berildi.", show_alert=True)
...
async def messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        ...
        if text == "📦 Ommaviy qo‘shish":
            ...
            await update.message.reply_text("Kodlarni probel bilan yuboring:"); return
        if step == "batch_codes":
    ...
    if text and not text.startswith("/"):
        ...
        else: await update.message.reply_text("❌ Bunday kodli kino topilmadi.")
