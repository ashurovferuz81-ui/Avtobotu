...
async def messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        if text == "🎬 Kino qo‘shish":
            ...
            await update.message.reply_text(f"📊 Jami foydalanuvchilar: {u_count} ta\n\nNecha kishiga reklama yubormoqchisiz? (Son yuboring):")
            return

        elif text == "📊 Statistika":
            u_count = cur.execute("SELECT COUNT(*) FROM users").fetchone()[0]
            m_count = cur.execute("SELECT COUNT(*) FROM movies").fetchone()[0]
            c_count = cur.execute("SELECT COUNT(*) FROM channels").fetchone()[0]
        ...
        if step == "rek_soni":
            if text.isdigit():
                ...
                if limit > total:
                    await update.message.reply_text(f"❌ Xato! Botda jami {total} ta user bor.")
                    context.user_data.clear()
                else:
                    context.user_data["rek_limit"] = limit
                    context.user_data["step"] = "rek_xabar"
                    await update.message.reply_text(f"✅ {limit} ta user tanlandi. Endi reklama xabarini yuboring:")
            else:
            ...
            sent = 0
            status_msg = await update.message.reply_text("🚀 Reklama yuborish boshlandi...")
            
            for u in users_list:
                try:
                    # copy_to bu xabarni (video, rasm, matn) asl holida yuboradi
                    await update.message.copy_to(chat_id=u[0])
                    sent += 1
                    if sent % 10 == 0:
                        await status_msg.edit_text(f"🚀 Yuborildi: {sent}/{limit}")
                    await asyncio.sleep(0.1) # Bot bloklanmasligi uchun
                except Exception:
                    pass
            
            await update.message.reply_text(f"🏁 Reklama yakunlandi.\n✅ Qabul qildi: {sent} ta user.", reply_markup=admin_keyboard())
            context.user_data.clear()
            return
            ...
            await update.message.reply_text("Endi kinoni (video/fayl) yuboring:")
            return

        elif step == "name":
            cur.execute("INSERT OR REPLACE INTO movies (code, file_id, name, views) VALUES(?,?,?,0)", 
                        (context.user_data["code"], context.user_data["file"], text))
            conn.commit()
            context.user_data.clear()
            await update.message.reply_text("✅ Kino bazaga saqlandi!", reply_markup=admin_keyboard())
            return
