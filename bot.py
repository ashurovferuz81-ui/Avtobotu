...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ch in all_channels:
        # FAQAT @ BILAN BOSHLANGANLARINI TEKSHIRAMIZ
        if ch.startswith("@"):
            try:
            ...
            # https:// bilan boshlanganlar tekshirilmaydi, lekin ro'yxatga qo'shiladi 
            # (agar foydalanuvchi hali "Obuna bo'ldim"ni bosmagan bo'lsa ko'rinishi uchun)
            if not is_callback:
                not_joined.append(ch)

    if not_joined:
        buttons = []
        for c in all_channels: # Hamma kanallarni tugma sifatida chiqaramiz
        ...
        if is_callback:
            # Callback bo'lganda faqat @ lilarni tekshiramiz, not_joined bo'sh bo'lmasa xato beradi
            # (Yuqoridagi loop callback vaqtida linklarni not_joined ga qo'shmaydi)
            await update.callback_query.answer("❌ Siz hali majburiy kanallarga a'zo bo'lmagansiz!", show_alert=True)
        else:
            await update.message.reply_text("Botdan foydalanish uchun quyidagi kanallarga a'zo bo'ling:", reply_markup=InlineKeyboardMarkup(buttons))
        return
    ...
    if is_callback:
        ...
        await context.bot.send_message(user_id, "🎬 Kino kodini yuborishingiz mumkin:")
    else:
...
async def messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_ID:
        ...
        if step == "one_name":
            ...
            conn.commit()
            await update.message.reply_text("✅ Saqlandi!", reply_markup=admin_keyboard())
            context.user_data.clear(); return

        # OMMAVIY QO'SHISH
        if text == "📦 Ommaviy qo‘shish":
            context.user_data["step"] = "batch_codes"
            await update.message.reply_text("Kodlarni probel bilan yuboring:")
            return
        if step == "batch_codes":
            codes = re.findall(r'\d+', text)
            if not codes: await update.message.reply_text("Xato!"); return
            context.user_data["b_codes"] = codes
            context.user_data["step"] = "batch_vids"
            await update.message.reply_text(f"✅ {len(codes)} ta kod. Videolarni tashlang."); return
        if step == "batch_vids" and update.message.video:
            ...
            if codes:
                ...
                if codes: await update.message.reply_text(f"✅ {c} saqlandi. Yana {len(codes)} ta...")
                    ...
                    await update.message.reply_text("🎉 Tugadi!", reply_markup=admin_keyboard())
                    context.user_data.clear()
            return
        ...
        if text == "📢 Kanal qo‘shish":
            context.user_data["step"] = "add_ch"; await update.message.reply_text("Kanalni kiriting (@kanal yoki link):"); return
        if step == "add_ch":
            cur.execute("INSERT OR IGNORE INTO channels VALUES(?)", (text,)); conn.commit()
            await update.message.reply_text("✅ Qo'shildi", reply_markup=admin_keyboard()); context.user_data.clear(); return

        if text == "❌ Kanal o‘chirish":
            context.user_data["step"] = "del_ch"; await update.message.reply_text("O'chiriladigan kanalni yozing:"); return
        if step == "del_ch":
            cur.execute("DELETE FROM channels WHERE channel=?", (text,)); conn.commit()
            await update.message.reply_text("❌ O'chirildi", reply_markup=admin_keyboard()); context.user_data.clear(); return
        ...
        if text == "📢 Reklama":
            ...
            await update.message.reply_text(f"Jami user: {total}\nSoni? (0 = hammasi)"); return
        if step == "wait_lim" and text.isdigit():
            context.user_data["limit"] = int(text); context.user_data["step"] = "wait_ad"
            await update.message.reply_text("Reklama xabari?"); return
        if step == "wait_ad":
