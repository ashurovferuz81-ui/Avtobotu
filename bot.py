...
def download_media(url, mode="video"):
    ...
    # YouTube bloklarini aylanib o'tish uchun maxsus sozlamalar
    ydl_opts = {
        'quiet': True,
        'no_warnings': True,
    ...
    if mode == "video":
        ydl_opts.update({
            # Eng yaxshi sifat, lekin 50mb dan oshmasligi uchun limit
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if mode == "audio":
            # MP3 bo'lsa nomini to'g'irlaymiz
...
async def start(m: types.Message):
    ...
    if await check_sub(m.from_user.id):
        ...
        await m.answer("✅ Bot faol! Link yuboring (Insta, TikTok, YouTube):", reply_markup=kb)
    else:
...
async def link_handler(m: types.Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🎬 Video", callback_data=f"v|{url}"),
         InlineKeyboardButton(text="🎵 MP3", callback_data=f"a|{url}")]
    ])
...
async def dl_callback(c: types.CallbackQuery):
    ...
    try:
        ...
        if "Sign in" in err_msg:
            txt = "❌ YouTube botni blokladi. Keyinroq urinib ko'ring."
        elif "filesize" in err_msg:
            txt = "❌ Video 50 MB dan katta. Telegram botlariga bu limit qo'yilgan."
        else:
            txt = "❌ Xatolik! Linkni tekshiring yoki video shaxsiyligini ko'ring."
        await msg.edit_text(txt)

# Statistika va Reklama (Oldingi koddagidek qoladi...)
